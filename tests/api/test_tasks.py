...
@pytest.mark.asyncio
async def test_change_task_status(client: TestClient, db_session: AsyncSession):
    ...
    task_response = client.post("/api/v1/tasks/", json=task_data, headers=headers)
    task_id = task_response.json()["id"]

    # Меняем статус задачи
    new_status = TaskStatus.IN_PROGRESS.value
    status_response = client.patch(f"/api/v1/tasks/{task_id}/status?status={new_status}", headers=headers)
...
@pytest.mark.asyncio
async def test_add_comment_to_task(client: TestClient, db_session: AsyncSession):
    ...
    comment_response = client.post(f"/api/v1/tasks/{task_id}/comments", json=comment_data, headers=headers)
